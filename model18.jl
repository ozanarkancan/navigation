...
function attention(states, attention_w, attention_v)
	h = hcat(states[5][2], states[7][end])
	hu = hcat(states[11], h)
	for i=2:(length(states[5])-1)
		hp = hcat(states[5][i+1], states[7][end-i+1])
		h = vcat(h, hp)
		hu = vcat(hu, hcat(states[11], hp))
	end
...
function encode(weight1_f, bias1_f, weight1_b, bias1_b, weight2_f, bias2_f, weight2_b, bias2_b,
	emb, state, words; dropout=false, pdrops=[0.5, 0.5])
	...
	for i=1:length(state[5])-1
		x = state[1][i]
		if dropout && pdrops[2] > 0.0
			x = x .* (rand!(similar(AutoGrad.getval(x))) .> pdrops[2]) * (1/(1-pdrops[2]))
		end
		state[5][i+1], state[6][i+1] = lstm(weight1_f, bias1_f, state[5][i], state[6][i], x)
		
		x = state[7][i]
		if dropout && pdrops[2] > 0.0
			x = x .* (rand!(similar(AutoGrad.getval(x))) .> pdrops[2]) * (1/(1-pdrops[2]))
		end
		state[7][i+1], state[8][i+1] = lstm(weight1_f, bias1_f, state[7][i], state[8][i], x)
	end
end

function decode(weight1, bias1, weight2, bias2, soft_w1, soft_w2, soft_w3, soft_b, state, x, mask, att; dropout=false, pdrops=[0.5, 0.5, 0.5])
	if dropout && pdrops[1] > 0.0
		x = x .* (rand!(similar(AutoGrad.getval(x))) .> pdrops[1]) * (1/(1-pdrops[1]))
	end

	state[9], state[10] = lstm(weight1, bias1, state[9], state[10], x)
	...
	state[11], state[12] = lstm2(weight2, bias2, state[9], state[10], inp, att)
	
	inp = state[11]
	if dropout && pdrops[3] > 0.0
		inp = inp .* (rand!(similar(AutoGrad.getval(inp))) .> pdrops[3]) * (1/(1-pdrops[3]))
	end
...
function loss(weights, state, words, views, ys, maskouts;lss=nothing, dropout=false, pdrops=[0.5, 0.5, 0.5], rewards=nothing)
	...
	encode(weights["enc_w1_f"], weights["enc_b1_f"], weights["enc_w1_b"], weights["enc_b1_b"],
		weights["enc_w2_f"], weights["enc_b2_f"], weights["enc_w2_b"], weights["enc_b2_b"],
		weights["emb_word"], state, words; dropout=dropout, pdrops=pdrops)

	state[9] = hcat(state[1][end], state[3][end])
	state[10] = hcat(state[2][end], state[4][end])
	state[11] = hcat(state[5][end], state[7][end])
	state[12] = hcat(state[6][end], state[8][end])
	...
	for i=1:length(views)
		...
		ypred = decode(weights["dec_w1"], weights["dec_b1"], weights["dec_w2"], weights["dec_b2"],
			weights["soft_w1"], weights["soft_w2"], weights["soft_w3"],
...
function train_pg(weights, prms, data, maps; args=nothing)
	...
	for (instruction, words) in data
		...
		while !stop
			...
			ypred = decode(weights["dec_w1"], weights["dec_b1"], weights["dec_w2"], weights["dec_b2"],
				weights["soft_w1"], weights["soft_w2"], 
				weights["soft_w3"], weights["soft_b"], state, x, mask, att)

			Xs = Xs == nothing ? state[11] : vcat(Xs, state[11])
...
function test(weights, data, maps; args=nothing)
	...
	for (instruction, words) in data
		...
		while !stop
			...
			att,att_s = attention(state, weights["attention_w"], weights["attention_v"])
			ypred = decode(weights["dec_w1"], weights["dec_b1"], weights["dec_w2"], weights["dec_b2"],
				weights["soft_w1"], weights["soft_w2"], weights["soft_w3"], weights["soft_b"], state, x, mask, att)
...
function initweights(atype, hidden, vocab, embed, window, onehotworld, numfilters; worldsize=[39, 39])
	...
	weights["enc_w2_f"] = xavier(Float32, hidden+hidden, 4*hidden)
	weights["enc_b2_f"] = zeros(Float32, 1, 4*hidden)
	weights["enc_b2_f"][1:hidden] = 1 # forget gate bias
	
	weights["enc_w2_b"] = xavier(Float32, hidden+hidden, 4*hidden)
	...
	weights["dec_w2"] = xavier(Float32, (hidden*2)+(hidden*2) + hidden*2, 4*hidden*2)
...
function initstate(atype, hidden, batchsize, length)
	...
	#forward
	state[5] = Array(Any, length+1)
	for i=1:(length+1); state[5][i] = convert(atype, zeros(batchsize, hidden)); end

	state[6] = Array(Any, length+1)
	for i=1:(length+1); state[6][i] = convert(atype, zeros(batchsize, hidden)); end
	
	#backward
	state[7] = Array(Any, length+1)
	for i=1:(length+1); state[7][i] = convert(atype, zeros(batchsize, hidden)); end

	state[8] = Array(Any, length+1)
	for i=1:(length+1); state[8][i] = convert(atype, zeros(batchsize, hidden)); end

	state[9] = convert(atype, zeros(batchsize, 2*hidden))
	state[10] = convert(atype, zeros(batchsize, 2*hidden))

	state[11] = convert(atype, zeros(batchsize, 2*hidden))
	state[12] = convert(atype, zeros(batchsize, 2*hidden))
