...
function loss(weights, state, words, views, ys, maskouts;lss=nothing, dropout=false, pdrops=[0.5, 0.5, 0.5])
	...
	encode(weights["enc_w1_f"], weights["enc_b1_f"], weights["enc_w1_b"], weights["enc_b1_b"],
		weights["emb_word"], state, words; pdrops=pdrops)
	...
	for i=1:length(views)
		...
		ypred = decode(weights["dec_w1"], weights["dec_b1"], weights["soft_w"], weights["soft_b"], state, x, maskouts[i], encoding; dropout=dropout, pdrops=pdrops)
