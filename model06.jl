...
@knet function model(x; dims=(21, 39, 25, 1), hidden=128, embed=128, actions=4)
	if !decode
		rembedding = wdot(x; out=embed)
		embedding = drop(rembedding; pdrop=0.2)
		input  = wbf3(embedding, h, h_2; out=hidden, f=:sigm)
	        forget = wbf3(embedding, h, h_2; out=hidden, f=:sigm)
	        output = wbf3(embedding, h, h_2; out=hidden, f=:sigm)
	        newmem = wbf3(embedding, h, h_2; out=hidden, f=:tanh)
	else
		embedding = spatial(x; dims=dims)
		input  = wbf3(embedding, h, h_2; out=hidden, f=:sigm)
	        forget = wbf3(embedding, h, h_2; out=hidden, f=:sigm)
	...
	if !decode
		input2  = wbf3(hdrop, h2, h2_2; out=hidden, f=:sigm)
	        forget2 = wbf3(hdrop, h2, h2_2; out=hidden, f=:sigm)
	        output2 = wbf3(hdrop, h2, h2_2; out=hidden, f=:sigm)
	        newmem2 = wbf3(hdrop, h2, h2_2; out=hidden, f=:tanh)
	else
		input2  = wbf3(hdrop, h2, h2_2; out=hidden, f=:sigm)
	        forget2 = wbf3(hdrop, h2, h2_2; out=hidden, f=:sigm)
	        output2 = wbf3(hdrop, h2, h2_2; out=hidden, f=:sigm)
