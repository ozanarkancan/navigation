...
@knet function model(x; dims=(21, 39, 25, 1), hidden=128, embed=128, actions=4)
	...
	if !decode
		input2  = wbf3(hdrop, h2, embedding; out=hidden, f=:sigm)
	        forget2 = wbf3(hdrop, h2, embedding; out=hidden, f=:sigm)
	        output2 = wbf3(hdrop, h2, embedding; out=hidden, f=:sigm)
	        newmem2 = wbf3(hdrop, h2, embedding; out=hidden, f=:tanh)
	else
		input2  = wbf3(hdrop, h2, embedding; out=hidden, f=:sigm)
	        forget2 = wbf3(hdrop, h2, embedding; out=hidden, f=:sigm)
	        output2 = wbf3(hdrop, h2, embedding; out=hidden, f=:sigm)
	...
	if decode
		return wbf2(h2, embedding; out=actions, f=:soft)
	end
end
