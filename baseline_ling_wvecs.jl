...
function loss(weights, state, words, acts, ys, maskouts;lss=nothing, dropout=false, pdrops=[0.5, 0.5, 0.5], rewards=nothing)
	total = 0.0; count = 0
	...
	#decode
	for i=1:length(acts)
		x = spatial(weights["emb_world"], acts[i])
...
function train(w, prms, data; args=nothing)
	...
	for (words, views, ys, maskouts) in data
		...
		g = lossgradient(w, state, words, acts, ys, maskouts; lss=nll, dropout=true, pdrops=args["pdrops"])
...
function train_loss(w, data; args=nothing)
	...
	for (words, views, ys, maskouts) in data
		...
		loss(w, state, words, acts, ys, maskouts; lss=nll, dropout=false, pdrops=args["pdrops"])
	...
	for (instruction, words) in data
		...
		while !stop
			...
			for i=1:length(models)
				...
				x = spatial(weights["emb_world"], preva)
