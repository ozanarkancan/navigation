...
function parse_commandline()
	...
	@add_arg_table s begin
		...
		("--window"; help = "size of the filter"; default = [3]; arg_type = Int; nargs = '+')
		("--filters"; help = "number of filters"; default = [30]; arg_type = Int; nargs = '+')
		...
		("--patience"; help = "patience param"; default = 20; arg_type = Int)
...
function execute(train_ins, test_ins, maps, vocab, emb, args; dev_ins=nothing)
    ...
    train_data = map(ins-> (ins, (emb != nothing ? ins_arr_embed(emb, vocab, ins.text) : ins_arr(vocab, ins.text))), vcat(train_ins[1], train_ins[2]))
    ...
    data = dev_ins != nothing ? map(x -> build_instance(x, maps[x.map], vocab; encoding=args["encoding"], emb=emb), dev_ins) : nothing
    dev_d = minibatch(data;bs=args["bs"])

    test_data = map(ins-> (ins, (emb != nothing ? ins_arr_embed(emb, vocab, ins.text) : ins_arr(vocab, ins.text))), test_ins)
    test_data_grp = map(x->map(ins-> (ins, (emb != nothing ? ins_arr_embed(emb, vocab, ins.text) : ins_arr(vocab, ins.text))),x), group_singles(test_ins))
