...
function execute(train_ins, test_ins, maps, vocab, emb, args; dev_ins=nothing)
	...
	dev_data = dev_ins != nothing ? map(ins -> (ins, ins_arr_embed(emb, vocab, ins.text)), dev_ins) : nothing
	dev_data_grp = dev_ins != nothing ? map(x -> map(ins-> (ins, ins_arr_embed(emb, vocab, ins.text)),x), group_singles(dev_ins)) : nothing
	...
	test_data = map(ins-> (ins, ins_arr_embed(emb, vocab, ins.text)), test_ins)
	#test_data_prg = map(ins-> (ins, ins_arr(d["vocab"], ins.text)), merge_singles(test_ins))
	test_data_grp = map(x->map(ins-> (ins, ins_arr_embed(emb, vocab, ins.text)),x), group_singles(test_ins))
	...
	for it=1:args["iterative"]
		...
		if args["pg"] != 0
			...
			prms_rl = initparams(w; args=args)
			append!(train_data, map(ins-> (ins, ins_arr_embed(emb, vocab, ins.text)), train_ins[2]))
			#args["greedy"] = false
