...
function attention(states, words, attention_w, attention_v)
...
function loss(weights, state, words, views, ys, maskouts;lss=nothing, dropout=false, pdrops=[0.5, 0.5, 0.5])
	...
	for i=1:length(views)
		...
		att,_ = attention(state, words, weights["attention_w"], weights["attention_v"])
...
function test(weights, data, maps; args=nothing)
	...
	for (instruction, words) in data
		...
		while !stop
			...
			att,att_s = attention(state, words, weights["attention_w"], weights["attention_v"])
...
function initweights(atype, hidden, vocab, embed, window, onehotworld, numfilters; worldsize=[39, 39])
	...
	weights["attention_w"] = xavier(Float32, vocab+hidden*2+hidden*2, hidden)
