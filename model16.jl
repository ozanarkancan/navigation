...
function loss(weights, state, words, views, ys, maskouts;lss=nothing, dropout=false, pdrops=[0.5, 0.5, 0.5], rewards=nothing)
	...
	for i=1:length(views)
		...
		ypred = decode(weights["dec_w1"], weights["dec_b1"], weights["dec_w2"], weights["dec_b2"], weights["soft_w1"], weights["soft_w2"], weights["soft_w3"],
...
function train_pg(weights, prms, data, maps; args=nothing)
	...
	for (instruction, words) in data
		...
		while !stop
			...
			ypred = decode(weights["dec_w1"], weights["dec_b1"], weights["dec_w2"], weights["dec_b2"], weights["soft_w1"], weights["soft_w2"], 
...
function initweights(atype, hidden, vocab, embed, window, onehotworld, numfilters; worldsize=[39, 39])
	...
	weights["dec_w2"] = xavier(Float32, hidden + hidden*2, 4*hidden)
	...
	weights["soft_w1"] = xavier(Float32, hidden, 4)
...
function initstate(atype, hidden, batchsize, length)
	...
	state[8] = convert(atype, zeros(batchsize, hidden))
