...
@knet function model(x; dims=(21, 39, 25, 1), hidden=128, embed=128, actions=4)
	if !decode
		input  = wbf2(x, h; out=hidden, f=:sigm)
	        forget = wbf2(x, h; out=hidden, f=:sigm)
	        output = wbf2(x, h; out=hidden, f=:sigm)
	        newmem = wbf2(x, h; out=hidden, f=:tanh)
	else
	...
	if decode
		hdrop = drop(h; pdrop=0.9)
		return wbf(hdrop; out=actions, f=:soft)
	end
end
