...
function spatial(filters1, bias1, filters2, bias2, filters3, bias3, filters4, bias4, emb, x)
	...
	c4 = sigm(conv4(filters4, c3; padding=0) .+ bias4)
	h = transpose(mat(c4))
	return h * emb
end
...
function loss(weights, state, words, views, ys, maskouts;lss=nothing, dropout=false, pdrops=[0.5, 0.5, 0.5])
	...
	for i=1:length(views)
		x = spatial(weights["filters_w1"], weights["filters_b1"], weights["filters_w2"], weights["filters_b2"], 
			weights["filters_w3"], weights["filters_b3"], weights["filters_w4"], weights["filters_b4"], 
			weights["emb_world"], views[i])
...
function initweights(atype, hidden, vocab, embed, window, onehotworld, numfilters; worldsize=[39, 39])
	...
	worldfeats = (worldsize[1] - sum(window) + length(window)) * (worldsize[2] - sum(window) + length(window)) * numfilters[4]
	...
	weights["filters_w4"] = xavier(Float32, window[4], window[4], numfilters[3], numfilters[4])
	weights["filters_b4"] = zeros(Float32, 1, 1, numfilters[4], 1)
